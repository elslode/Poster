    ...
    public static ArrayList<Movie> getMovieFromJSON(JSONObject jsonObject) {
        ...
        try {
            ...
            for(int i = 0; i < jsonArray.length(); i++) {
                ...
                String posterPath = BASE_POSTER_URL + SMOLL_POSTER_SIZE + objectMovie.getString(KEY_POSTER_PATH);
                ...
                String dataRelease = objectMovie.getString(KEY_RELEASE_DATE);
                Movie movie = new Movie(id, voteCount, title, originalTitle, overview, posterPath, bigPosterPath , backgroundPath, voteAverage, dataRelease);
                result.add(movie);
            }
        } catch (JSONException e) {
            e.printStackTrace();
